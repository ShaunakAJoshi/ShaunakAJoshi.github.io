---
layout: page
title: Projects
permalink: /projects/
---

Here is a list of projects I have worked on (in no particular order):

* [Keyword Spotting]({{site.url}}/projects/kws)
* [Hangman]({{site.url}}/projects/hangman)
* [FindIt]({{site.url}}/projects/findit)
* [RISC (EE224 course project)]({{site.url}}/projects/risc)
* [Bandit Algorithms (IE613 course project)]({{site.url}}/projects/bandit)
* [IPL analysis]({{site.url}}/projects/ipl)
* [Handwriting Recognition Pen]({{site.url}}/projects/itsp)